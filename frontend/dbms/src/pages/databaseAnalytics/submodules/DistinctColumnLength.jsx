...
const DistinctColumnLength = () => {
    ...
    const [distinctCount, setDistinctCount] = useState('');
    ...
    useEffect(() => {
        ...
        dispatch(updateProjectDetails({ field: "subModule", value: "Distinct Column Length" }));
    }, []);
    useEffect(() => {
        if (projectDetails.subModule === 'Distinct Column Length') {
            fetchPrev();
            fetchValues();
        }
    }, [projectDetails]);
    ...
    const fetchPrev = useCallback(async () => {
        try {
            ...
            setPreSaveSettings(fetchedData.distinctCount || '');
        } catch (error) {
    ...
    const fetchValues = async () => {
        try {
            ...
            const data = response.data.settings['distinctLength'];
            if (data) {
                setDefaultSettings(data.distinctCount || '');
            }
        }
        catch (error) {
    ...
    useEffect(() => {
        if (preSaveSettings) {
            setDistinctCount(preSaveSettings);
        } else if (defaultSettings) {
            setDistinctCount(defaultSettings);
        }
    ...
    const callAction = async () => {
        ...
        try {
            const response = await axios.post(`${API_URL}/api/databaseAnalyticsRoute/distinctColumnLength`, formData, {
                headers: {
            ...
            <main className={styles.moduleContent}>
                <div className={styles.optionBox}>
                    <div className={styles.optionGroup}>
                        <label htmlFor="distinctCount" className={styles.settingsLabel}>
...
export default DistinctColumnLength