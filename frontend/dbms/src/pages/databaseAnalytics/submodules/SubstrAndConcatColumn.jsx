...
const SubstrAndConcatColumn = () => {
  ...
  useEffect(() => {
    ...
    dispatch(updateProjectDetails({ field: "subModule", value: "Substr and Concat Columns" }));
  }, []);
  useEffect(() => {
    if (projectDetails.subModule === 'Substr and Concat Columns') {
      fetchPrev();
      fetchValues();
    }
  }, [projectDetails]);
  ...
  const fetchValues = async () => {
    try {
      ...
      const data = response.data.settings['substrConcatColumns'];
      if (data) {
  ...
  const callAction = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/databaseAnalyticsRoute/substrConcatColumns`, formData, {
        headers: {
...
export default SubstrAndConcatColumn