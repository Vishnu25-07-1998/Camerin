...
const ColumnCharEachCount = () => {
    ...
    const [ColumnCharSettings, setColumnCharSettings] = useState({
        distinctCount: '',
        maxCharLoop: '',
    });
    ...
    useEffect(() => {
        ...
        dispatch(updateProjectDetails({ field: "subModule", value: "Column Each Char Count" }));
    }, []);
    useEffect(() => {
        if (projectDetails.subModule === 'Column Each Char Count') {
            fetchPrev();
        }
    }, [projectDetails]);

    useEffect(() => {
        if (preSaveSettings?.distinctCount || preSaveSettings?.maxCharLoop) {
            setColumnCharSettings(preSaveSettings);
        } else if (defaultSettings?.distinctCount || defaultSettings?.maxCharLoop) {
            setColumnCharSettings(defaultSettings);
        }
    ...
    const fetchPrev = useCallback(async () => {
        try {
            ...
            setPreSaveSettings({
                distinctCount: fetchedData.distinctCount || '',
                maxCharLoop: fetchedData.maxCharLoop || ''
            });
    ...
    const fetchValues = async () => {
        try {
            const response = await fetch(`${API_URL}/api/datasourceroute/getSettingsData`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${authState.token}`,
            ...
            const data = responseData.settings?.columnCharEachCount;

            if (data) {
                setDefaultSettings({
                    distinctCount: data.distinctCount || '',
                    maxCharLoop: data.maxCharLoop || ''
                });
            }
        } catch (error) {
    ...
    const callAction = async () => {
        ...
        try {
            const response = await axios.post(`${API_URL}/api/databaseAnalyticsRoute/columnEachCharCount`, formData, {
                headers: {
            ...
            <main className={styles.moduleContent}>
                <div className={styles.optionBox}>
                    <div className={styles.optionGroup}>
                        ...
                        <input
                            ...
                            min="0"
                            placeholder="e.g., 5"
                            value={ColumnCharSettings.distinctCount || ''}
                            onChange={(e) => setColumnCharSettings(prev => ({ ...prev, distinctCount: e.target.value }))}
                        />
                    </div>
                    <div className={styles.optionGroup}>
                        <label htmlFor="maxLoop" className={styles.settingsLabel}>
                            Max char loop per column
                        </label>
                        <input
                            type="number"
                            id="maxLoop"
                            ...
                            min="0"
                            placeholder="e.g., 5"
                            value={ColumnCharSettings.maxCharLoop || ''}
                            onChange={(e) => setColumnCharSettings(prev => ({ ...prev, maxCharLoop: e.target.value }))}
                        />
                    </div>
                </div>
                ...
                {tableData.length > 0 && <div className="relation-table module-table">
                    <table>
                        <thead>
                            <tr>
                                {Object.keys(tableData[2]).map((header, idx) => <th key={idx}>{header}</th>)}
                            </tr>
                        </thead>
                        <tbody>
                            {
...
export default ColumnCharEachCount