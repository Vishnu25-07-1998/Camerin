...
const UniqueIndexes = () => {
    ...
    const [uniqueIndexSettings, setUniqueIndexSettings] = useState({
        max_no_of_unique_keys_required: '',
        max_no_of_columns_in_unique_key: '',
        max_key_combinations_to_be_checked: '',
        max_key_combinations_identified: ''
    });
    ...
    useEffect(() => {
        ...
        dispatch(updateProjectDetails({ field: "subModule", value: "Unique Indexes" }));
    }, []);
    useEffect(() => {
        if (projectDetails.subModule === 'Unique Indexes') {
            fetchPrev();
            fetchValues();
        }
    }, [projectDetails]);
    ...
    const fetchPrev = useCallback(async () => {
        try {
            ...
            setPreSaveSettings({
                max_no_of_unique_keys_required: fetchedData.maxUniqueKeys || '',
                max_no_of_columns_in_unique_key: fetchedData.maxColumnsInKey || '',
                max_key_combinations_to_be_checked: fetchedData.maxKeyCombosCheck || '',
                max_key_combinations_identified: fetchedData.maxKeyCombosIdentified || ''
            });
        } catch (error) {
     ...
     const fetchValues = async () => {
        try {
          ...
          const data = response.data.settings['uniqueKeys'];
          if (data) {
            setDefaultSettings({ max_no_of_unique_keys_required: data.maxUniqueKeys || '',
              max_no_of_columns_in_unique_key: data.maxColumnsInKey || '',
              max_key_combinations_to_be_checked: data.maxKeyCombosCheck || '',
              max_key_combinations_identified: data.maxKeyCombosIdentified || ''
            });
          }
      ...
      useEffect(() => {
        if (preSaveSettings?.max_no_of_unique_keys_required || preSaveSettings?.max_no_of_columns_in_unique_key || preSaveSettings?.max_key_combinations_to_be_checked || preSaveSettings?.max_key_combinations_identified) {
          setUniqueIndexSettings(preSaveSettings);
        } else if (defaultSettings?.max_no_of_unique_keys_required || defaultSettings?.max_no_of_columns_in_unique_key || defaultSettings?.max_key_combinations_to_be_checked || defaultSettings?.max_key_combinations_identified) {
          setUniqueIndexSettings(defaultSettings);
        }
    ...
    const callAction = async () => {
        ...
        try {
            const response = await axios.post(`${API_URL}/api/databaseAnalyticsRoute/uniqueIndexes`, formData, {
                headers: {
            ...
            <main className={styles.moduleContent}>
                <div className={styles.optionBox}>
                    <div className={styles.optionGroup}>
                        <label htmlFor="uniqueKey" className={styles.settingsLabel}>
                            max_no_of_unique_keys_required
                        </label>
                        <input
                            type="number"
                            ...
                            min="0"
                            placeholder="e.g., 5"
                            value={uniqueIndexSettings.max_no_of_unique_keys_required}  
                            onChange={(e) => setUniqueIndexSettings({ ...uniqueIndexSettings, max_no_of_unique_keys_required: e.target.value })}
                        />
                    </div>

                    <div className={styles.optionGroup}>
                        <label htmlFor="uniqueColumns" className={styles.settingsLabel}>
                            max_no_of_columns_in_unique_key
                        </label>
                        <input
                            type="number"
                            ...
                            min="0"
                            placeholder="e.g., 5"
                            value={uniqueIndexSettings.max_no_of_columns_in_unique_key}
                            onChange={(e) => setUniqueIndexSettings({ ...uniqueIndexSettings, max_no_of_columns_in_unique_key: e.target.value })}
                        />
                    </div>
                    <div className={styles.optionGroup}>
                        <label htmlFor="keyCombine" className={styles.settingsLabel}>
                            max_key_combinations_to_be_checked
                        </label>
                        <input
                            type="number"
                            ...
                            min="0"
                            placeholder="e.g., 5"
                            value={uniqueIndexSettings.max_key_combinations_to_be_checked}
                            onChange={(e) => setUniqueIndexSettings({ ...uniqueIndexSettings, max_key_combinations_to_be_checked: e.target.value })}
                        />
                    </div>
                    <div className={styles.optionGroup}>
                        <label htmlFor="keyidentify" className={styles.settingsLabel}>
                            max_key_combinations_identified
                        </label>
                        <input
                            type="number"
                            ...
                            min="0"
                            placeholder="e.g., 5"
                            value={uniqueIndexSettings.max_key_combinations_identified}
                            onChange={(e) => setUniqueIndexSettings({ ...uniqueIndexSettings, max_key_combinations_identified: e.target.value })}
                        />
                    </div>
                </div>
