...
const Cardinality = () => {
    ...
    useEffect(() => {
        ...
        dispatch(updateProjectDetails({ field: "subModule", value: "Cardinality" }));
    }, []);
    useEffect(() => {
        if (projectDetails.subModule === 'Cardinality') {
            fetchPrev();
        }
    }, [projectDetails]);
    ...
    const callAction = async () => {
        ...
        try {
            const response = await axios.post(`${API_URL}/api/databaseAnalyticsRoute/cardinality`, formData, {
                headers: {
