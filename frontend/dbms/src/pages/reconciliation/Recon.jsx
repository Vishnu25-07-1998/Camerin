...
const Recon = () => {
    ...
    const changeHandler = (event, name) => {
        ...
        setEntities(prev => ({ ...prev, [name]: file }));
        ...
        Papa.parse(file, {
            ...
            complete: (result) => {
                ...
                } else if (name === 'rightEntity') {
                    setRightHeaders(parsedHeaders);
                }
            },
            error: (err) => {
                console.error(`Error parsing ${name} file:`, err);
            }
        });
    };
    ...
    useEffect(() => {
        if (leftHeaders.length > 0 && rightHeaders.length > 0) {
            const common = leftHeaders.filter(header => rightHeaders.includes(header));
            setHeaders(common);
        }
    ...
    const fetchOutput = async () => {
        try {
            const response = await axios.post(
                `${API_URL}/api/reconRouter/dbOuts`,
                { selectedModule },
                {
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const callAction = async () => {
        ...
        const endpoints = {
            "Generate Recon Report": `${API_URL}/api/reconRouter/generateReconReport`,
        };
            ...
            <main className="module-content relation-module">
                <div className={styles["module-top"]}>
                    ...
                    <div className={styles["select_wrap"]}>
                        ...
                        <MultiSelect options={headers} onChange={(e) => handleHeaders(e, "leftEntity")} selectedValue={selectedHeaders.leftEntity} placeholder={'key for comparison between entities'} />
                    </div>
                    <div className={styles["select_wrap"]}>
                        <label className={styles.label}>Choose common columns (for non key comparison)</label>
                        <MultiSelect options={headers} onChange={(e) => handleHeaders(e, "rightEntity")} selectedValue={selectedHeaders.rightEntity} placeholder={'for non key comparison'} />
                    </div>
                </div>
