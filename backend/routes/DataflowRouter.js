...
const getUploadDirs = (uploadType) => {
    ...
    switch (uploadType) {
        case 'dataFlow':
            return {
                left: path.join(base, 'dataflowDiagram', 'dataFlow', 'Inputs'),
                output: path.join(base, 'dataflowDiagram', 'dataFlow', 'Outputs')
            }
        default:
...
router.post('/dbOuts', authMiddleware, async (req, res) => {
    try {
        ...
        const outputDirectory = path.join(__dirname, '..', 'uploads', 'modules', 'dataflowDiagram', moduleFolder, 'Outputs');
...
router.post('/dataFlow', authMiddleware, setUploadType('dataFlow'), removeOldFiles, upload.array("files"), async (req, res) => {
    try {
        const files = req.files;
        // Define paths
        const baseModulePath = path.join(__dirname, '..', 'uploads', 'modules', 'dataflowDiagram', 'dataFlow');
        ...
        const scriptPath = path.join(baseModulePath, 'Scripts', 'ERD_v4.py');
        ...
        res.status(200).json({ message: "success", imgSrc: path.join('uploads', 'modules', 'dataflowDiagram', 'dataFlow', 'Outputs', 'er_diagram.png') });
