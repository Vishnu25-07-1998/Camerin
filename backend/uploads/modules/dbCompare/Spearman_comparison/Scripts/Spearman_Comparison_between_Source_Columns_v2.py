...
import io
from Settings_Component_Spearman_ColumnNames import *
...
def get_column_relations(s1, s2):
    ...
    corr_spearman_details = s1_series.corr(s2_series, method='spearman')
    return corr_spearman_details
...
if __name__ == "__main__":
    ...
    #folder_output_name_module = "\\Spearman_Column_Names_Compare"
    ...
    #folder_output_name_module_for_filename = ".Spearman_Column_Names_Compare."
    #output_file_extn = ".xlsx"
    #sheet_name = "Spearman_Column_Names_Compare"
    
    column_relation_details_pd = pd.DataFrame(columns=['Table_Name_From', 'Column_Name_From', 'Table_Name_To', 'Column_Name_To', 'Spearman_value'])
    
    folder_new_group_name = group_name_left + "_and_" + group_name_for_filename_right
    ...
    for filename_from in tqdm(glob.glob(os.path.join(folder_path_for_input_left, '*.csv'))):
        with codecs.open(filename_from, 'r', encoding='utf-8', errors='ignore') as file_from:
            ...
            tic = time.time()
            print(f"Table '{filename_from}' correlation Column Name Spearman Match in progress")
            ...
            for col_i in (df_input_from.columns):
                ...
                for filename_all in (glob.glob(os.path.join(folder_path_for_input_right, '*.csv'))):
                    with codecs.open(filename_all, 'r', encoding='utf-8', errors='ignore') as file_all:
                        ...
                        df_input_all = pd.read_csv(file_all, nrows=1)
            ...
            column_relation_details_pd = pd.DataFrame(relation_details_row,columns=['Table_Name_From', 'Column_Name_From', 'Table_Name_To', 'Column_Name_To', 'Spearman_Value'])
