...
import io
from Settings_Component_QuickRatio_ColumnNames import *
...
if __name__ == "__main__":
    ...
    #folder_output_name_module = "\\Quick_ratio_Column_Names_Compare"
    ...
    for filename_from in tqdm(glob.glob(os.path.join(folder_path_for_input_left, '*.csv'))):
        with codecs.open(filename_from, 'r', encoding='utf-8', errors='ignore') as file_from:
            ...
            tic = time.time()
            print(f"Table '{filename_from}' correlation Column Name Quick_ratio Match in progress")
