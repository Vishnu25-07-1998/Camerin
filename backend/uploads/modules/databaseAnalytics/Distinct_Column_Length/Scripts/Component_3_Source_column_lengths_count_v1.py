...
import time
import xlsxwriter
from Settings_Component_3 import *
    ...
    for filename in tqdm(glob.glob(os.path.join(folder_path_for_input, star_csv_var))):
        with codecs.open(filename, 'r', encoding='utf-8', errors='ignore') as file:
            ...
            for col_i in tqdm(df_input.columns):
                ...
                col_num_str="Column " + str(col_num)
...
def report_distinct_vals(df_input, col_i, count_of_distinct_values_per_column):
    ...
    column_distinct_val_details1 = df_input_col_i.groupby([col_i])[col_i].agg([count_var]).sort_values([count_var,col_i], ascending=[False, True]).reset_index()
    ...
    column_distinct_val_details_appended = df_input_null_count_per_column_df._append(column_distinct_val_details1)
